...
def polarized_ideal_classes(O, F):
    ...
    EXAMPLES::
        ...
        True
        sage: p = polarized_ideal_classes(s[1], 5) # long time
        sage: len(p) # long time
        2
        sage: p # long time and random
...
def ideal_order(basis, K):
    ...
    EXAMPLES::
        ...
        Maximal Order generated by a in Number Field in a with defining polynomial x^2 - 15 with a = 3.872983346207417?
        sage: O1.basis()
        [1, a]
...
def are_nn_isogenous(Z1, Z2, n, F1, F2, transformation=False, double_check=False):
    ...
    Some checks for the are_nn_isogenous function::
        ...
        sage: # long time
        sage: are_nn_isogenous(Zmax, Zmax, 5, 5, 5, transformation=True)
        (True, ...)
        sage: are_nn_isogenous(Zmax, Zmax, 1, 1, 1, transformation=True)
        (True, 1)
        sage: are_nn_isogenous(Zother, Zother, 1, F, F, transformation=True)
        (True, 1)
        sage: are_nn_isogenous(Zmax, Zother, 3, 1, F)
        False
